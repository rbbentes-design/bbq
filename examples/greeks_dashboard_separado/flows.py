...
def compute_flow_score(leveraged_flow, buyback_daily=0, cot_net_change=0,
    ...
    # Calibrado para: ±1σ = valor normal, ±3σ = extremo, saturação em 3σ
    ...
    # Historico (se disponivel) tem precedencia. Caso contrario, usa blackout_pct
    # para inferir: 0% blackout = janela cheia (Z baseado em escala), 100% blackout = Z = -3.
    if history_buyback is not None:
        z_buyback = flow_zscore(buyback_daily, history_buyback)
    elif buyback_blackout_pct >= 0.5:
        # Blackout >= 50%: forca Z negativo proporcional
        # (50% = -1.5, 80% = -2.4, 100% = -3)
